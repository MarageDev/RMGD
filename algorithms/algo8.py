...
@torch.no_grad()
def algo7(D_train, patchsize=3, stride=1, N=50, schedule='linear', device='cpu', mask_weight_type="standard", renoise_factor=0.1, save_immediatly_to_cpu=True, seed=None):
    if seed is not None:
...
if __name__ == "__main__":
    ...
    a5 = algo7(multi_res_tensors, patchsize=patchsize, stride=stride, N=50, device=device, mask_weight_type=mode, schedule='linear', renoise_factor=0.2, save_immediatly_to_cpu=False, seed=1)
    print(a5[-1].shape)
