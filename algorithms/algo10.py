...
def algo8(D_train, patchsize=3, stride=1, N=50, schedule='linear', device='cpu', mask_weight_type="standard", renoise_factor=0.1, save_immediatly_to_cpu=True, seed=None):
    ...
    for s in range(scales):
        ...
        if s == 0: 
            ...
            x_noise = ((L @ torch.randn(C*H*W, device=device)).view(1, C, H, W) + mu)*t0 + (1-t0)*torch.randn(1, C, H, W, device=device) * (1. - t0) # TODO CHANGEMENT
            x_n1 = x_noise
...
if __name__ == "__main__":
    ...
    plt.imsave(f"results/algo_9_conv_mosaic_{seed.item()}.png",tensor_to_numpy_img((comparison[1]+1)/2))
    plt.show()