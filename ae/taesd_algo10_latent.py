...
def algo8(D_train, patchsize=3, stride=1, N=50, schedule='linear', device='cpu', mask_weight_type="standard", renoise_factor=0.1, save_immediatly_to_cpu=True, seed=None):
    ...
    for s in range(scales):
        ...
        if s == 0: 
            ...
            x_noise = ((L @ torch.randn(C*H*W, device=device)).view(1, C, H, W) + mu)*t0 + torch.randn(1, C, H, W, device=device) * (1. - t0) # TODO CHANGEMENT
            x_n1 = x_noise
            times = make_times(N*(1-t0), schedule, t0=t0) # TODO CHANGEMENT
...
def transform_to_latent_space(tensors:torch.TensorType, taesd:TAESD, dev):
    """
    ...
    encoded_tensor_list = []
    for scale, tensor in enumerate(tensors):

        tensor_image_view = tensor[0] 
        image_raw = tensor_image_view.unsqueeze(0).to(dev)
        image_enc = taesd.encoder(image_raw)
        
        current_scale_tensor = torch.zeros(tensor.shape[0],*image_enc.shape[1:])
        current_scale_tensor[0] = image_enc
        
        for idx in tqdm(range(1,tensor.shape[0]),desc=f"Converting to encoded tensor (scale {scale}/{len(tensors)})"):
            tensor_image_view = tensor[idx] 
            image_raw = tensor_image_view.unsqueeze(0).to(dev)
            image_enc = taesd.encoder(image_raw)
            current_scale_tensor[idx] = image_enc
            
        encoded_tensor_list.append(current_scale_tensor)
        
    return encoded_tensor_list
def imgs_to_gif(path,imgs):
    from PIL import Image
    final_size = imgs[-1].shape[-2:]
    
    resized_imgs = [F.interpolate(t, size=final_size, mode="nearest") for t in imgs if t is not None]
    
    np_imgs = [np.uint8(np.clip(img.permute(0, 2, 3, 1).detach().numpy()[0] , 0, 1) * 255) for img in resized_imgs]
    im_list = [Image.fromarray(np_img, mode='RGB') for np_img in np_imgs]
    im_list[-1].save(path, save_all=True, append_images=im_list[1:], duration=1, loop=0)
...
if __name__ == "__main__":
    ...
    encoded_tensors = transform_to_latent_space(multi_res_tensors, taesd, dev=device)
    seed = torch.randint(0,99999999,(1,1))
    a5 = algo8(
        encoded_tensors, patchsize=patchsize, stride=stride, 
        N=150, device=device, mask_weight_type=mode, schedule='linear', 
        renoise_factor=0.3, save_immediatly_to_cpu=False, seed=None
        )
    image_dec = decode_tensor(a5[-1].to(device), taesd).cpu()

    a5_clean = [decode_tensor(img.to(device), taesd).cpu() for img in a5 if img is not None]
    a5_lat = [img.to(device).cpu() for img in a5 if img is not None]
    plt.tight_layout()
    
    plt.figure(1)
    plt.title("Generated Image")
    plt.imshow(TF.to_pil_image(image_dec[0]))
    ...
    plt.imshow(tensor_to_numpy_img(comparison[1]))
    
    plt.imsave(f"results/algo_9_conv_mosaic_{seed.item()}.png",tensor_to_numpy_img(comparison[1]))
    
    imgs_to_gif("out.gif",a5_clean)
    imgs_to_gif("out_lat.gif",a5_lat)
    plt.show()